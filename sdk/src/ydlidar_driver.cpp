...
namespace ydlidar{
	...
	YDlidarDriver::~YDlidarDriver(){
		{
			ScopedLocker lock(_scanning_lock);
			isScanning = false;
		}
    ...
    result_t YDlidarDriver::waitPackage(node_info * node, uint32_t timeout) {
		...
		if(package_Sample_Index == 0) {
			...
			while ((waitTime=getms() - startTs) <= timeout) {
				...
				for (size_t pos = 0; pos < recvSize; ++pos) {
					...
					case 8:
						...
						break;
					case 9:
						CheckSun += (currentByte<<LIDAR_RESP_MEASUREMENT_ANGLE_SAMPLE_SHIFT);
						break;
					}
			...
			if(PackagePaidBytes == recvPos){
				startTs = getms();
				recvPos = 0;
				int package_sample_sum = package_Sample_Num*PackageSampleBytes;
				while ((waitTime=getms() - startTs) <= timeout) {
					size_t remainSize = package_sample_sum - recvPos;
					size_t recvSize;
                    result_t ans =waitForData(remainSize, timeout-waitTime, &recvSize);
					if (ans != RESULT_OK){
						delete[] recvBuffer;
					...
					for (size_t pos = 0; pos < recvSize; ++pos) {
						if(m_intensities){
							if(recvPos%3 == 2){
								Valu8Tou16 += (recvBuffer[pos]<<LIDAR_RESP_MEASUREMENT_ANGLE_SAMPLE_SHIFT);
		...
		if(CheckSunResult == true){
			...
			if((*node).distance_q2/4 != 0){
				AngleCorrectForDistance = (int32_t)((atan(((21.8*(155.3 - ((*node).distance_q2/4)) )/155.3)/((*node).distance_q2/4)))*3666.93);
			}else{
	...
	void YDlidarDriver::simpleScanData(std::vector<scanDot> *scan_data , node_info *buffer, size_t count) {
		...
		for (int pos = 0; pos < (int)count; ++pos) {
			...
			dot.distance = buffer[pos].distance_q2/4.0f;
